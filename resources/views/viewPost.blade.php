...
@foreach($blogComments as $c=>$a)   
        <?php $bandera = 0 ?>
            ...
            <strong>{{$a->name}} :</strong> {{ $a->comments }}
            </div>
			</div>
        <!--<div class="row">
            ...
            <div  id="reply-comment-{{$c}}" class="col-7  " >
...
@endforeach
 ...
 <div class="col-12 media mb-4">
        <div class="col-8">
            <form action="{{route('post.comment')}}" method="POST" name="comment_post" id="comment_post">
                @csrf
                ...
                <div class="form-group">
					...
					<input type="hidden" class="form-control" @if(!empty(Auth::id())) value="{{Auth::user()->name}}" readonly="readonly" @endif id="login_check" name="login_check">
                </div>
                <div class="form-group">
                    <label for="email">Email *</label>
                    <input type="email" class="form-control" id="email" @if(!empty(Auth::id())) value="{{Auth::user()->email}}" readonly="readonly" @endif name="email" required>
                </div>
                <div class="form-group">
...
<script type="text/javascript">   
	...
	{
	alert("Please Login to processed");
